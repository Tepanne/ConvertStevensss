...
// Fungsi untuk menggabungkan file teks
document.getElementById('mergeButton').addEventListener('click', function() {
  ...
  const reader = new FileReader();
  let uniqueLines = new Set(); // Untuk memastikan tidak ada duplikasi kontak
  let uniqueChars = new Set(); // Untuk memastikan tidak ada duplikasi huruf
  let mergedContent = '';
  ...
  const readFile = (index) => {
    if (index >= files.length) {
      ...
      // Log aktivitas
      logActivity('Menggabungkan file TXT dan menghapus duplikasi kontak dan huruf');
      return;
    }
    ...
    reader.onload = function(e) {
      ...
      lines.forEach(line => {
        ...
        const uniqueCharsInLine = new Set(line);
        if (uniqueCharsInLine.size === line.length) {
